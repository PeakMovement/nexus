...
// Use better-sqlite3 directly for seeding since Prisma v7 client is ESM-only
const dbPath = path.join(__dirname, "..", "dev.db");

// Actually, let's use prisma's own client. We need to use the generated client correctly.
// For Prisma 7 with output to src/generated/prisma, we need to handle ESM.
// Simplest approach: use SQL directly for seeding.
...
try {
  ...
  // Fall back to executing SQL via prisma db execute
  console.log("Using SQL file approach for seeding...");
  seedWithSQL();
  process.exit(0);
}
...
function seedDirect(db) {
  console.log("Seeding food database with better-sqlite3...");
  db.exec("DELETE FROM Food");

  const insert = db.prepare(`
    INSERT INTO Food (id, name, category, proteinPer100g, carbsPer100g, fatPer100g, caloriesPer100g, pricePerKg, unit, store, isAffordable, isPremium)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertMany = db.transaction((foods) => {
    for (const food of foods) {
      insert.run(
        ...
        food.proteinPer100g, food.carbsPer100g, food.fatPer100g, food.caloriesPer100g,
        food.pricePerKg, food.unit || "kg", food.store || null,
        food.isAffordable ? 1 : 0, food.isPremium ? 1 : 0
      );
    }
  });
  insertMany(foods);
...
function seedWithSQL() {
  const foods = getFoods();
  const fs = require("fs");
  let sql = "DELETE FROM Food;\n";
  for (const food of foods) {
    const id = generateId();
    const store = food.store ? `'${food.store}'` : "NULL";
    sql += `INSERT INTO Food (id, name, category, proteinPer100g, carbsPer100g, fatPer100g, caloriesPer100g, pricePerKg, unit, store, isAffordable, isPremium) VALUES ('${id}', '${food.name}', '${food.category}', ${food.proteinPer100g}, ${food.carbsPer100g}, ${food.fatPer100g}, ${food.caloriesPer100g}, ${food.pricePerKg}, '${food.unit || "kg"}', ${store}, ${food.isAffordable ? 1 : 0}, ${food.isPremium ? 1 : 0});\n`;
  }
...
function getFoods() {
  return [
    ...
    // FAT SOURCES
    { name: "Sunflower Oil", category: "fat", proteinPer100g: 0, carbsPer100g: 0, fatPer100g: 100, caloriesPer100g: 884, pricePerKg: 30, store: "Shoprite", isAffordable: true, isPremium: false },
    ...
    { name: "Olive Oil", category: "fat", proteinPer100g: 0, carbsPer100g: 0, fatPer100g: 100, caloriesPer100g: 884, pricePerKg: 120, store: "Woolworths", isAffordable: false, isPremium: true },
    // VEGETABLES
    ...
    // DAIRY
    { name: "Full Cream Milk", category: "dairy", proteinPer100g: 3.3, carbsPer100g: 5, fatPer100g: 3.3, caloriesPer100g: 61, pricePerKg: 18, store: "Shoprite", isAffordable: true, isPremium: false },
    { name: "Maas (Amasi)", category: "dairy", proteinPer100g: 3.5, carbsPer100g: 4, fatPer100g: 3, caloriesPer100g: 56, pricePerKg: 15, store: "Shoprite", isAffordable: true, isPremium: false },
